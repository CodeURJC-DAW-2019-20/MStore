...
// Move skins folder to dist/css

gulp.task('move:skins', () => {
  return gulp.src([
      path.src_skins + '/*'
    ])
    .pipe(gulp.dest(path.dist_css + '/skins'));
});
...
gulp.task(
  'default',
  gulp.series('clean', gulp.parallel('concat:js', 'move:js', 'concat:css', 'move:css', 'move:skins', 'uglify:js', 'pug', 'sass:minified', 'sass:expanded'), 'watch')
);
